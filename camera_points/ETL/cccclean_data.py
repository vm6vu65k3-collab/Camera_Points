...
from camera_points.DataBase.database import engine
from .create_ssl import build_ssl_context, SSLContextAdapter
from .clean_helper import df_to_records, clean_region_name

#===========
# Config
#===========
BASE_DIR = Path(__file__).resolve().parents[0]
DATA_DIR = BASE_DIR / "raw_datas"

OPEN_DATA_CSV_URL = "https://opdadm.moi.gov.tw/api/v1/no-auth/resource/api/dataset/EA5E6FCD-B82D-43B7-A5CF-E9893253187E/resource/965B8213-AAC2-4C62-9811-868500E6C1F4/download"

EXPECTED = [
    "city_name", "region_name", "address",
    "dept_name", "branch_name", 
    "longitude", "latitude", "direct", "speed_limit"
]
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        print(f"[WARN] 缺失欄位：{missing} 目前欄位：{df.columns.tolist()}")
...
# 先新增 etl_runs，取得id
# 新增 raw_data 資料
# 用更新插入方式 camera_points，已存在就更新
# 更新 etl_runs 尚缺的欄位

def insert_etl_run_with_own_transaction(engine, rows_fetched: int) -> int:
    payload = {
    ...
    meta = MetaData()
    table = Table('etl_runs', meta, autoload_with = engine)
...
def update_etl_with_own_transaction(
        engine, status: str, 
        etl_run_id: int,
        rows_inserted: int,
        rows_updated: int,
...
def load_all(
    ...
    etl_run_id = insert_etl_run_with_own_transaction(
                    engine, rows_fetched = 0 if df_raw is None else len(df_raw))
    ...
    try:
        with engine.begin() as conn:
            if df_raw is not None or not df_raw.empty:
                df_raw_to_insert = df_raw.copy()
                df_raw_to_insert['etl_run_id'] = etl_run_id
                raws_inserted = insert_raw_data(conn, df_raw_to_insert, chunk_size)
            
            if df_raw is not None or not df_raw.empty:
                df_camera_to_insert = df_raw.copy()
                camera_processed = upsert_camera_points(conn, df_camera_to_insert, chunk_size)
        
        update_etl_with_own_transaction(
            engine,
            status        = "success",
        ...
        print(f"[INFO] ETL匯入完成")

        return {
            "etl_run_id"      : etl_run_id,
            "raws_inserted"   : raws_inserted,
...
def main():
    csv_path = download_csv(OPEN_DATA_CSV_URL, DATA_DIR, None)
    raw_df = load_csv(csv_path)
    df = clean_data(raw_df)
    
    load_all(engine, df, chunk_size = 500)
